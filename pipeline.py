...
Executes all SQL models using DuckDB (in-process SQL engine, no server needed)
and writes results to analysis/ as CSVs and prints a summary report.
...
# DuckDB runs entirely in-memory — no database server needed.
# It reads CSVs directly via read_csv_auto() in SQL.
con = duckdb.connect()
...
print("[2/4] Running retailer performance mart...")
...
print("[3/4] Running warehouse OTIF mart...")
...
print("[4/4] Running reason code analysis...")
reason_sql = """
SELECT
    reason_code,
    reason_description,
...
print("\n── By Quarter ──")
q_view = exec_kpis[["fiscal_quarter","gross_deductions","invalid_rate_pct",
                     "deduction_rate_pct","recovery_rate_pct","trend_direction"]].copy()
